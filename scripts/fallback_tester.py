...
from app.routing.router import call_with_fallback, FallbackRouter
...
class FallbackTester:
    ...
    def setup_mock_scenario(self, scenario: str):
        ...
        if scenario == "basic_success":
            ...
            self.mock_providers["cerebras"] = MockProvider("cerebras", fail_mode="http_500")
            self.mock_providers["nahcrof"] = MockProvider("nahcrof")

        elif scenario == "logical_model_fallback":
            # All primary routes fail, fallback to another logical model
            self.mock_providers["cerebras"] = MockProvider("cerebras", fail_mode="http_500")
            self.mock_providers["nahcrof"] = MockProvider("nahcrof", fail_mode="http_500")
            # glm-4.5 routes work
            ...
            self.mock_providers["cerebras"] = MockProvider("cerebras", fail_mode="timeout", delay=0.1)
            self.mock_providers["nahcrof"] = MockProvider("nahcrof")

        elif scenario == "all_fail":
            # All routes fail
            self.mock_providers["cerebras"] = MockProvider("cerebras", fail_mode="http_500")
            self.mock_providers["nahcrof"] = MockProvider("nahcrof", fail_mode="connection")
    ...
    async def execute_route(self, resolved_route: ResolvedRoute) -> Dict[str, Any]:
        ...
        if not provider:
            raise Exception(f"No mock provider configured for {resolved_route.provider}")
        ...
        self.call_log.append({
            ...
            "api_key": resolved_route.api_key[:8] + "..." if resolved_route.api_key else None,
        })

        print(f"[Attempt] {resolved_route.source_logical_model} -> {resolved_route.provider}/{resolved_route.model}")
    ...
    async def test_fallback(self, logical_model: str, scenario: str) -> Dict[str, Any]:
        ...
        print(f"\n{'='*60}")
        print(f"Testing fallback routing for '{logical_model}' with scenario '{scenario}'")
        print(f"{'='*60}")
        ...
        try:
            ...
            return {
                ...
                "errors": [{"attempt": a.attempt_number, "error": str(err)} for a, err in zip(e.attempted_routes, e.errors)],
                "call_log": self.call_log.copy()
            }
    ...
    def show_call_log(self):
        ...
        for i, call in enumerate(self.call_log, 1):
            print(f"{i:2d}. {call['logical_model']} -> {call['provider']}/{call['model']} ({call['wire_protocol']})")


def main():
    parser = argparse.ArgumentParser(description="Test multi-level model fallback routing")
    parser.add_argument("logical_model", nargs="?", help="Logical model name to test (e.g., glm-4.6)")
    parser.add_argument(
        "--scenario",
        choices=["basic_success", "api_key_fallback", "provider_fallback", "logical_model_fallback", "timeout_fallback", "all_fail"],
        default="basic_success",
        help="Test scenario to run"
    )
    parser.add_argument("--list-models", action="store_true", help="List available logical models")
    ...
    try:
        config = config_loader.load_config(args.logical_model)
