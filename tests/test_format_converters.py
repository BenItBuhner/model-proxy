...
def test_openai_to_anthropic_request_with_tool_calls_and_results():
    ...
    openai_req = {
        ...
        "messages": [
            {"role": "assistant", "content": None, "tool_calls": [
                {"id": "call_1", "type": "function", "function": {"name": "add", "arguments": json.dumps({"a": 1, "b": 2})}}
            ]},
    ...
    assert any(block for block in assistant_message["content"] if isinstance(block, dict) and block.get("type") == "tool_use")
...
def test_anthropic_to_openai_response_basic():
    ...
    assert openai_resp["model"] == "claude-3-opus"
...
def test_response_round_trip():
    ...
    anthropic_resp_2 = openai_to_anthropic_response(openai_resp, "claude-3-opus")
