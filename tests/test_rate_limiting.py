...
def app():
    ...
    app.add_middleware(RateLimitingMiddleware, requests_per_minute=5, tokens_per_minute=1000)
...
def test_rate_limiting_allows_requests(client):
    ...
    response = client.get("/test", headers={"Authorization": f"Bearer {CLIENT_API_KEY}"})
...
def test_rate_limiting_blocks_excess_requests(client):
    ...
    response = client.get("/test", headers={"Authorization": f"Bearer {CLIENT_API_KEY}"}, follow_redirects=False)
...
def test_rate_limiting_skips_health_checks(client):
    ...
    for _ in range(10):
        ...
        assert response.status_code in [200, 503]  # Health check might fail if DB not available
