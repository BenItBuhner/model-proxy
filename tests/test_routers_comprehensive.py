...
def test_openai_endpoint_with_all_parameters(client, mock_env_vars, mock_openai_response):
...
def test_anthropic_endpoint_with_all_parameters(client, mock_env_vars, mock_anthropic_response):
...
def test_anthropic_endpoint_multiple_messages(client, mock_env_vars, mock_anthropic_response):
...
def test_openai_endpoint_streaming_chunks(client, mock_env_vars):
    ...
    with patch("app.providers.openai_provider.OpenAIProvider.call_stream") as mock_stream_func:
...
def test_anthropic_endpoint_streaming_chunks(client, mock_env_vars):
    ...
    with patch("app.providers.anthropic_provider.AnthropicProvider.call_stream") as mock_stream_func:
        mock_stream_func.return_value = mock_stream()
...
def test_openai_endpoint_cross_provider_tool_conversion(client, mock_env_vars, mock_anthropic_response):
...
def test_anthropic_endpoint_cross_provider_tool_conversion(client, mock_env_vars, mock_openai_response):
...
def test_openai_endpoint_model_name_preserved(client, mock_env_vars, mock_openai_response):
...
def test_anthropic_endpoint_model_name_preserved(client, mock_env_vars, mock_anthropic_response):
    """Test that requested model name is preserved in response."""
    with patch("app.providers.anthropic_provider.AnthropicProvider.call", new_callable=AsyncMock) as mock_call:
        mock_call.return_value = mock_anthropic_response
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["model"] == "claude-3-opus"  # Should preserve requested model name
