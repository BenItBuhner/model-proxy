    ...
    try:
        ...
        for provider_name, config in configs.items():
            ...
            if "endpoints" in config and "base_url" not in config["endpoints"]:
                errors.append(
                    f"Provider '{provider_name}' missing base_url in endpoints"
                )
            ...
            if "authentication" in config:
                ...
                if "header_name" not in auth:
                    errors.append(
                        f"Provider '{provider_name}' missing header_name in authentication"
                    )
        ...
        for provider_name in configs.keys():
            if is_provider_enabled(provider_name):
                ...
                if len(available_keys) == 0:
                    warnings.append(
                        f"Provider '{provider_name}' is enabled but has no API keys configured"
                    )
                else:
...
def validate_model_config() -> Tuple[bool, str]:
    ...
    """
    try:
        # Use the routing config loader (config/models/*.json) instead of the
        # legacy flat models.json resolver.
