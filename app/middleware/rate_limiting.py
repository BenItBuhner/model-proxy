...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, requests_per_minute: int = None, tokens_per_minute: int = None
    ):
        super().__init__(app)
    ...
    def _cleanup_old_entries(self, client_key: str):
        ...
        self._request_history[client_key] = [
            (ts, tokens)
            for ts, tokens in self._request_history[client_key]
            if ts > one_minute_ago
        ]

    def _check_rate_limit(
        self, client_key: str, estimated_tokens: int = 0
    ) -> Tuple[bool, str]:
        """
        ...
        if request_count >= self.requests_per_minute:
            return (
                False,
                f"Rate limit exceeded: {request_count}/{self.requests_per_minute} requests per minute",
            )
        ...
        if total_tokens >= self.tokens_per_minute:
            return (
                False,
                f"Rate limit exceeded: {total_tokens}/{self.tokens_per_minute} tokens per minute",
            )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        response.headers["X-RateLimit-Remaining-Tokens"] = str(
            max(0, self.tokens_per_minute - total_tokens)
        )
